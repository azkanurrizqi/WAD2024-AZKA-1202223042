...
class AuthController
{
        ...
        if (isset($_POST['submit'])) {
            ...
            if (password_verify($password, $data_mahasiswa['password'])){
                ...
                if (isset($input["remember"])) {
                    setcookie("id", $data_mahasiswa['id'], time() + (86400 * 30), "/");
                } else {
                ...
                $_SESSION['message'] = "Login Gagal atau NIM Password Salah";
    ...
    private function generateNIM($id_pendaftaran){
        ...
        $query = "SELECT * FROM users WHERE nim = '$id_pendaftaran'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) == 1) {
    ...
    public function register_step_2() 
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->generateNIM($id_pendaftaran) ['$nim']  
        ...
        $nama = $_POST['$data_pendaftaran'];
        $jurusan = $_POST['$data_pendaftaran'];
        if(mysqli_num_rows($result) == 0) {
            $query = "INSERT INTO users  VALUES ('' , 'nim', 'id_pendaftaran', 'password', 'nama', 'jurusan')";
            $result_insert = mysqli_query($conn, $query);
            if($insert){
                $_SESSION ['message'] = 'Register Berhasil, Silahkan Login';
                $_SESSION ['color'] = 'success';
                unset('$id_pendaftaran')
